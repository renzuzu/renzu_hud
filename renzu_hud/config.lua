...
config = {}
config.gamebuild = 2060 -- if 2189 pedshot transparent will not work, we will use normal pedshot ( gamebuild is what you set on your server start example: +set sv_enforceGameBuild 2189, available build 1604, 2060, 2189 and more.) this is important if you are using UI Normal with Ped Face.
config.framework = 'ESX' -- ESX | VRP | QSHIT | STANDALONE
config.weight_type = false -- ESX item weight or limit type
config.ESX_Items = { -- important to change job, weight. (job = false means its available to use for everyone)
...
config.statusordering = { -- offset is additional 35px per status, 275 default is the starting from health status. -- SET enable = false to disable the status
...
--UTILS UI SETTINGS -- the default settings is optimized for CPU usage, (DEFAULT: false,false,0ms,'')
config.useSomeGPU_transition = true -- use some gpu resource to some UI animation and effects?
config.status_hardwareaccelerated = true -- more smoother animation and UI but use more cpu usage.
config.animation_ms = '500ms' -- animation delay ( this affects cpu usage of animations )
config.transition = 'ease' -- ease, linear, or leave it like = '' (blank)
-- BODY STATUS
config.bodystatus = true -- ENABLE BODY STATUS FUNCTION AND UI?
config.checkbodycommandjob = 'ambulance' -- allowed jobs to use to command
...
function minigame_carjack() return 
	exports["cd_keymaster"]:StartKeyMaster() -- change this to your mini game (EXPORTS ONLY not events)
end
...
config.engine_job = 'mechanic' -- Allowed Jobs
